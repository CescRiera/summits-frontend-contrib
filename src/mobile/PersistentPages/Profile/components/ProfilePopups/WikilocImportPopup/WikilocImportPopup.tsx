...
import styles from "./WikilocImportPopup.module.css";
...
const WikilocImportPopup: React.FC<WikilocImportPopupProps> = ({ isOpen, onClose }) => {
  const { t } = useI18n();
  ...
  useEffect(() => {
    if (!isOpen) return;
    reset();
    trackEvent("popup_view", "wikiloc_import_popup");
  ...
  const handleSearch = async () => {
    trackEvent("button_click", "wikiloc_import_search");
    if (!searchQuery.trim()) {
      setStatus("error");
  ...
  const handleSelectUser = (wikilocUser: WikilocUser) => {
    trackEvent("interaction", `wikiloc_import_select_user_${wikilocUser.id}`);
  ...
  const handleImport = async () => {
    ...
    trackEvent("button_click", "wikiloc_import_submit");
    setStatus("importing");
    setStatusMessage("");
    setProgressMessage("");
    try {
  ...
  const handleClose = () => {
    trackEvent("button_click", "wikiloc_import_popup_close");
    onClose();
  };
  ...
  return (
    <AppModal
      open={isOpen}
      onClose={() => {
        trackEvent("button_click", "wikiloc_import_popup_overlay_close");
        onClose();
      }}
      variant="dialog"
    ...
    >
      <div className={styles["wikiloc-popup__header"]}>
        <h2 className={`${styles["wikiloc-popup__title"]} typography-title-medium`}>
          {t("profile.wikilocImportTitle")}
        </h2>
        <p className={`${styles["wikiloc-popup__description"]} typography-body-medium`}>
      ...
      <div className={styles["wikiloc-popup__search-section"]}>
        <div className={styles["wikiloc-popup__search-row"]}>
          <input
            className={`${styles["wikiloc-popup__input"]} typography-body-small`}
            type="text"
        ...
        {selectedUser && (
          <div className={styles["wikiloc-popup__selected-user"]}>
            ...
            <button
              type="button"
              className={`${styles["wikiloc-popup__clear-btn"]} typography-button-medium`}
              onClick={() => {
                setSelectedUser(null);
                setWikilocUsers([]);
                setStatus("idle");
        ...
        {wikilocUsers.length > 0 && !selectedUser && (
          <div className={styles["wikiloc-popup__results"]}>
            <span className={`${styles["wikiloc-popup__results-label"]} typography-body-small`}>
            ...
            {wikilocUsers.map((wikilocUser) => (
              ...
              >
                ...
                {wikilocUser.trailCount && (
                  <span className={`${styles["wikiloc-popup__trails"]} typography-body-small`}>
      ...
      {statusMessage && (
        <div className={`${styles["wikiloc-popup__status"]} typography-body-small`}>
          {renderStatusIcon()}
      ...
      {progressMessage && (
        <div className={`${styles["wikiloc-popup__progress"]} typography-body-small`}>
          {progressMessage}
        </div>
      )}
      ...
      <div className={styles["wikiloc-popup__actions"]}>
        <button
          className={`${styles["wikiloc-popup__button"]} ${styles["wikiloc-popup__button--secondary"]} typography-button-medium`}
          onClick={handleClose}
        >
          {t("common.close")}
        </button>
        {!isImporting && status !== "completed" && (
          <button
            className={`${styles["wikiloc-popup__button"]} typography-button-medium`}
...
export default WikilocImportPopup;
