...
import styles from "./ManageClubsModal.module.css";
  ...
  return (
    ...
    >
      <div className={styles["manage-clubs__manage-modal-header"]}>
        <h3 className={`${styles["manage-clubs__section-title"]} typography-title-medium`}>
      ...
      <div className={styles["manage-clubs__manage-modal-body"]}>
        {actionError ? (
          <div className={`${styles["manage-clubs__error"]} typography-body-small`} style={{ marginBottom: 12 }}>
            {actionError}
          </div>
        ) : null}
        ...
        {createdClubs.length > 1 ? (
          <CustomDropdown
            options={createdClubs.map((club) => ({ id: club.id, name: club.name }))}
            value={managedClub?.id ?? null}
            onChange={(val) => setManagedClubId(val as number)}
            placeholder={t("clubs.actions.selectClub") || "Select a club"}
            showSearch={false}
            showClear={false}
            className={styles["manage-clubs__custom-dropdown"] || ""}
          />
        ) : null}
        ...
        {managedClub ? (
          <button
            type="button"
            className={`${styles["manage-clubs__button"]} typography-button-small`}
            onClick={() => {
        ...
        <div>
          <strong className="typography-title-small">
            {t("clubs.tabs.requests") || "Requests"}
          </strong>
          {pendingRequests.length === 0 ? (
            <p className={`${styles["manage-clubs__muted"]} typography-body-small`}>
              ...
              {pendingRequests.map((request) => (
                <div key={request.user_id} className={styles["manage-clubs__list-item"]}>
                  {request.user_image ? (
                    ...
                    <div className={`${styles["manage-clubs__avatar"]} typography-label-medium`}>
                      {request.user_name.charAt(0).toUpperCase()}
                    </div>
                  )}
                  <div className={styles["manage-clubs__list-body"]}>
                    <strong className="typography-body-small">{request.user_name}</strong>
                  </div>
                  <div className={styles["manage-clubs__list-actions"]}>
                    ...
                    <button
                      type="button"
                      className={`${styles["manage-clubs__button"]} ${styles["manage-clubs__button--secondary"]} typography-button-small`}
                      onClick={() =>
                        managedClub &&
              ...
              {members
                ...
                .map((member) => (
                  <div key={member.user_id} className={styles["manage-clubs__list-item"]}>
                    ...
                    <div className={styles["manage-clubs__list-body"]}>
                      <strong className="typography-body-small">{member.user_name}</strong>
                    </div>
                    <button
                      type="button"
                      className={`${styles["manage-clubs__button"]} ${styles["manage-clubs__button--danger"]} typography-button-small`}
                      onClick={() =>
                        managedClub &&
