...
import styles from "./StatsScroller.module.css";
...
const StatCard: React.FC<{ item: StatItem }> = ({ item }) => (
  <div className={styles["stats-scroller__item"]}>
    ...
    <div className={styles["stats-scroller__content"]}>
      <div
        className={`${styles["stats-scroller__value"]} typography-title-medium`}
        style={{ color: item.color }}
      >
        {item.value}
      </div>
      <div
        className={`${styles["stats-scroller__label"]} typography-label-medium`}
      >
        {item.label}
      </div>
    </div>
