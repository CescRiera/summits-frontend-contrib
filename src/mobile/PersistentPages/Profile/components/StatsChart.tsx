...
import { timeStringToSeconds } from "../../../utils/timeFormatting";
import {
  formatStatDuration,
  formatStatInteger,
...
const StatsChart: React.FC<StatsChartProps> = ({
  ...
  const { formatStatDistance, formatStatElevationGain, distanceUnit, elevationUnit } = useUnitFormat();

  // Translation function for metric config
  const getMetricConfig = (metricKey: string) => {
    const configs: Record<string, any> = {
      distance: {
        label: `${t("userStats.metrics.distance")} (${distanceUnit})`,
        color: "#3b82f6",
        formatter: (value: number) => formatStatDistance(value),
      },
      elevation_gain: {
        label: `${t("userStats.metrics.elevationGain")} (${elevationUnit})`,
        color: "#10b981",
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        <XAxis dataKey="date" hide={true} stroke="rgba(255, 255, 255, 0.6)" />
        <YAxis
        ...
        />
        <Legend
          wrapperStyle={{ color: "rgba(255, 255, 255, 0.8)", bottom: "-5px" }}
