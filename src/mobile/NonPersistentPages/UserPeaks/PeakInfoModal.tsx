...
import styles from "./PeakInfoModal.module.css";

interface PeakInfoModalProps {
  isOpen: boolean;
  peakId: number | null;
  onClose: () => void;
  onPeakDeleted: () => void;
...
const PeakInfoModal: React.FC<PeakInfoModalProps> = ({
  isOpen,
  peakId,
  onClose,
  onPeakDeleted,
  ...
  const handleDeletePeak = async () => {
    ...
    try {
      ...
      while (keepGoing) {
        ...
        if (result.peak_removed || result.remaining_ascensions <= 0) {
          keepGoing = false;
        }
      }
      onPeakDeleted();
      ...
      <>
        ...
        <div className={styles["peak-info-modal__header"]}>
          ...
          <div className={styles["peak-info-modal__header-text"]}>
            <h2 className={`${styles["peak-info-modal__peak-name"]} typography-title-medium`}>{peak.name}</h2>
            <div className={styles["peak-info-modal__peak-meta"]}>
              <span className={`${styles["peak-info-modal__elevation-badge"]} typography-label-medium`}>
                {formatMeters(peak.elevation)}
              </span>
              <span className={`${styles["peak-info-modal__ascension-badge"]} typography-label-medium`}>
        ...
        <div className={styles["peak-info-modal__body"]}>
          {ascensions.length > 0 && (
            <div className={styles["peak-info-modal__section"]}>
              <h3 className={`${styles["peak-info-modal__section-title"]} typography-label-small`}>
                ...
                return (
                  <div key={`${asc.route_id ?? "unlinked"}-${idx}`}>
                    <div className={styles["peak-info-modal__ascension"]}>
                      <span className={`${styles["peak-info-modal__ascension-num"]} typography-title-large`}>
                        {idx + 1}.
                      </span>
                      <div className={styles["peak-info-modal__ascension-details"]}>
                        {asc.route_name ? (
                          asc.route_url ? (
                            ...
                            >
                              <p className={`${styles["peak-info-modal__ascension-name"]} typography-body-medium`}>
                                {asc.route_name}
                              </p>
                            </a>
                          ) : (
                        ...
                        {ascDate && (
                          <p className={`${styles["peak-info-modal__ascension-date"]} typography-body-small`}>{ascDate}</p>
                        )}
                      </div>
          ...
          {ascensions.length === 0 && !loading && (
            <div className={`${styles["peak-info-modal__empty"]} typography-body-small`}>
        ...
        <div className={styles["peak-info-modal__footer"]}>
          <button className={`${styles["peak-info-modal__close-btn"]} typography-label-medium`} onClick={onClose}>
            {t("userPeaks.peakInfo.close")}
          </button>
          <button
            className={`${styles["peak-info-modal__delete-peak-btn"]} typography-label-medium`}
