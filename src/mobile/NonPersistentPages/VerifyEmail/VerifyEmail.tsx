...
import { verifyEmail, resendVerificationCode } from "../../../shared/api/endpoints/user";
...
export default function VerifyEmail() {
  ...
  const handleCodeComplete = async (completeCode: string) => {
    ...
    if (!emailToUse) {
      setError(t("auth.emailVerification.codeRequired") || "Please enter your email address.");
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        ...
        t("auth.emailVerification.codeError");
      ...
      <div className={styles["verify-email"]}>
        ...
        <div className={styles["verify-email__content"]}>
          <div className={styles["verify-email__card"]}>
            <div className={styles["verify-email__icon-container"]}>
              <Check size={64} className={styles["verify-email__icon"]} />
            </div>
            
            <h2 className={`${styles["verify-email__title"]} typography-title-large`}>
              {t("auth.emailVerification.authenticated")}
            </h2>
            
            <button
              className={`${styles["verify-email__gmail-button"]} typography-button-medium`}
    ...
    return (
      <div className={styles["verify-email"]}>
        ...
        <div className={styles["verify-email__content"]}>
          <div className={styles["verify-email__card"]}>
            <div className={styles["verify-email__icon-container"]}>
              <X size={64} className={styles["verify-email__icon"]} />
            </div>
  ...
  return (
    <div className={styles["verify-email"]}>
      ...
      <div className={styles["verify-email__content"]}>
        <div className={styles["verify-email__card"]}>
          <div className={styles["verify-email__icon-container"]}>
            <Mail size={64} className={styles["verify-email__icon"]} />
          </div>
          ...
          <p className={`${styles["verify-email__message"]} typography-body-medium`}>
            {email
          ...
          <div className={styles["verify-email__code-section"]}>
            <p className={`${styles["verify-email__code-label"]} typography-body-small`}>
          ...
          {error && !resendSuccess && (
            <div className={`${styles["verify-email__error"]} typography-body-small`}>
              {error}
            </div>
          )}
          ...
          {(email || manualEmail.trim()) && (
            <button
              type="button"
              className={`${styles["verify-email__resend-button"]} typography-body-medium`}
          ...
          {!email && (
            <>
              ...
              <button
                className={`${styles["verify-email__login-button"]} typography-body-medium`}
                onClick={handleGoToLogin}
              >
