...
export const MapProvider: React.FC<MapProviderProps> = ({ children }) => {
  ...
  const [activeFilter, setActiveFilter] =
    useState<MapFilterType>(initialFilter);
  const [pendingFilter, setPendingFilter] = useState<MapFilterType | null>(
    null
  );
  ...
  const handleSelectList = useCallback(
    async (listId: number | null) => {
      ...
      if (listId === null || currentListId === listId) {
        ...
        // Fetch GeoJSON before applying filter to ensure it's available
  ...
  const handleToggleMapFilters = useCallback(() => {
    ...
    setMapFilters((prev) => ({
      ...prev,
      showMapFilters: !prev.showMapFilters,
    }));
  }, []);
