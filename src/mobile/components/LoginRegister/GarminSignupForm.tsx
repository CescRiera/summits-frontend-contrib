...
import styles from "./GarminSignupForm.module.css";
...
export function GarminSignupForm({ showTitle = true }: GarminSignupFormProps) {
  ...
  const handleMobileOAuth = async (oauthUrl: string) => {
    try {
        ...
        if (url.startsWith("summitstracker://oauth-callback")) {
          try {
            const urlObj = new URL(
              url.replace("summitstracker://", "https://")
            );
            const success = urlObj.searchParams.get("success") === "true";
            const email = urlObj.searchParams.get("email");
            const requiresVerification =
              urlObj.searchParams.get("requiresVerification") === "true";
      ...
      deepLinkListenerRef.current = await App.addListener(
        "appUrlOpen",
        handleDeepLink
      );
      ...
      await cleanupOAuth();
      setError("Failed to open browser for Garmin Connect signup.");
      setLoading(false);
    }
  };
  ...
  return (
    <form className={styles["garmin-form"]} onSubmit={handleSubmit}>
      {showTitle && (
        <h3 className={`${styles["garmin-form__title"]} typography-title-medium`}>
          {t("auth.register.garmin.title")}
        </h3>
      )}
      <div className={`${styles["garmin-form__notice"]} typography-body-small`}>
        {t("auth.register.garmin.warning")}
      </div>
      {error && (
        <div
          className={`${styles["garmin-form__error"]} typography-body-small`}
        >
          {error}
        </div>
      )}
      ...
      <label className={styles["garmin-form__checkbox"]}>
        <input
          type="checkbox"
          checked={acceptTerms}
