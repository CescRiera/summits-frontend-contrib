// MapLayers.ts - Optimized Version
import type { Map as MapboxMap, FilterSpecification } from "mapbox-gl";
import type { PeakListWithPeaks, UserPeaksWithGeoJSON, ListPeaksGeoJSONResponse } from "../../../shared/api/types";
import { PEAK_ICONS, SHELTER_ICONS, getElevationRangeFilter } from "./MapUtils";
import {
  applyPeakLabelPaint,
...
export class MapLayerManager {
  ...
  private fitToBounds(bounds: BoundsCoordinates): void {
    ...
    try {
      this.map.fitBounds(mapBounds, FIT_BOUNDS_OPTIONS);
    } catch (error) {
  ...
  private updateTileLayerFilters(): void {
    ...
    if (this.isNearbyPeaksEnabled) {
      const sourceKey = this.currentVisualizationMode === "user" 
        ? `user-${this.currentUserPeaks?.user_id}` 
        : `list-${this.currentListPeaks?.list_id}`;
      
      if (this.lastExclusionSource === sourceKey) {
        idsToExclude = this.cachedExcludedIds;
      } else {
        if (this.currentVisualizationMode === "user" && this.currentUserPeaks) {
          ...
          idsToExclude = this.currentListGeoJson.features
            .map((f) => Number(f.properties.id))
            .filter((id) => !isNaN(id));
        }
        this.cachedExcludedIds = idsToExclude;
        this.lastExclusionSource = sourceKey;
      }
    } else {
      this.cachedExcludedIds = [];
      this.lastExclusionSource = null;
    }
