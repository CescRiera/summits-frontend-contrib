...
export interface SuuntoAuthOptions {
  onSuccess: (tokens: {
...
export const initiateSuuntoLogin = async (options: SuuntoAuthOptions) => {
  ...
  const oauthUrl = `${backendUrl}/api/auth/suunto?platform=${platform}&language=${encodeURIComponent(language)}`;
...
const handleWebOAuth = (oauthUrl: string, options: SuuntoAuthOptions) => {
  ...
  messageHandler = async (event: MessageEvent) => {
    ...
    if (event.data && event.data.type === "oauth-success") {
      ...
      const error = event.data.message?.error || "An error occurred during Suunto login.";
      await cleanupOAuth();
      onError(error);
      onLoading(false);
    }
...
const handleMobileOAuth = async (oauthUrl: string, options: SuuntoAuthOptions) => {
  const { onSuccess, onError, onLoading } = options;
  try {
    ...
    await cleanupOAuth();
    onError("Failed to open browser for Suunto login.");
    onLoading(false);
  }
};
