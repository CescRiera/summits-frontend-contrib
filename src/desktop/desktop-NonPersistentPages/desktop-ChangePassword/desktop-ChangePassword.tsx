...
import styles from "./desktop-ChangePassword.module.css";
...
export default function ChangePassword() {
  ...
  const handleSubmitInternal = async (emailToUse: string, codeToUse: string) => {
    ...
    try {
      ...
      setError(
        (err as any)?.response?.data?.error || t("auth.resetPassword.error")
      );
    } finally {
      setLoading(false);
    }
  ...
  if (success) {
    return (
      <div className={styles["change-password"]}>
        <div className={styles["change-password__header"]}>
          <button
            className={`${styles["change-password__back-button"]} typography-label-large`}
        ...
        <div className={styles["change-password__content"]}>
          <div className={styles["change-password__card"]}>
            <div className={styles["change-password__icon-container"]}>
              <CheckCircle
                size={80}
                className={styles["change-password__icon"]}
              />
            </div>
            <h2
              className={`${styles["change-password__title"]} typography-desktop-title-large`}
            >
              {t("auth.resetPassword.successTitle")}
            </h2>
            <p
              className={`${styles["change-password__message"]} typography-desktop-body-medium`}
            >
              {t("auth.resetPassword.successMessage")}
            </p>
            <button
              className={`${styles["change-password__login-button"]} typography-desktop-body-medium`}
              onClick={handleGoToLogin}
            >
  ...
  return (
    <div className={styles["change-password"]}>
      ...
      <div className={styles["change-password__content"]}>
        <div className={styles["change-password__card"]}>
          <div className={styles["change-password__icon-container"]}>
            <KeyRound size={80} className={styles["change-password__icon"]} />
          </div>
          <h2
            className={`${styles["change-password__title"]} typography-desktop-title-large`}
          >
          ...
          <div className={styles["change-password__code-section"]}>
            <p className={`${styles["change-password__code-label"]} typography-desktop-body-small`}>
          ...
          {error && !resendSuccess && (
            <div
              className={`${styles["change-password__error"]} typography-desktop-label-medium`}
            >
              {error}
            </div>
          )}
          ...
          >
            ...
            />
            <button
              className={`${styles["change-password__button"]} typography-desktop-button-medium`}
              type="submit"
          ...
          {(email || manualEmail.trim()) && (
            <button
              type="button"
              className={`${styles["change-password__resend-button"]} typography-desktop-body-medium`}
