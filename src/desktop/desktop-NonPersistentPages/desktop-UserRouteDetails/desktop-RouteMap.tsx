...
import styles from "./desktop-RouteMap.module.css";
import {
  MAPBOX_ACCESS_TOKEN,
  PEAK_ICONS,
...
import { usePeakSynchronization } from "./desktop-usePeakSynchronization.ts";
...
interface RouteMapProps {
  ...
  onPeakSelect?: (
    peakId: number,
    peakData?: { name: string; name_en?: string | null; elevation: number }
  ) => void;
...
<path d="M6.79078 9.81555C6.79078 9.81555 7.34865 9.62581 8.47007 10.178C9.59149 10.7302 9.97098 11.8459 11.0924 12.3981C12.2138 12.9503 12.7717 12.7606 12.7717 12.7606L14.9804 8.27487C14.9804 8.27487 14.4226 8.46461 13.3012 7.91242C12.1797 7.36023 11.8002 6.24449 10.6788 5.69231C9.5574 5.14012 8.99953 5.32986 8.99953 5.32986L6.79078 9.81555ZM6.79078 9.81555L5.50234 12.4322" stroke="#000000" stroke-linecap="round" stroke-linejoin="round"></path>
    ...
    useEffect(() => {
      ...
      const map = new mapboxgl.Map({
        ...
        preserveDrawingBuffer: true, // Required for canvas video recording (route animation)
      ...
      map.on("style.load", () => {
        ...
        if (isGlobeEnabledRef.current !== undefined) {
          currentMap.setProjection(
            isGlobeEnabledRef.current ? "globe" : "mercator"
          );
        }
      ...
      map.scrollZoom.setWheelZoomRate(4.0); // Increased from default 1.0
      ...
      map.on("load", async () => {
        ...
        // Notify parent that the map is fully loaded and ready
        // This is critical for enabling the route animation button (isMapReady state)
        if (onMapRef) {
          onMapRef(map);
        }
      ...
      map.on("click", async (e: MapMouseEvent) => {
        ...
        if (features.length === 0) {
          // Empty click - close peak details if open
          if (onClosePeakDetails) {
            onClosePeakDetails();
          }
          return;
        }
        ...
        if (topFeature.layer?.id === LAYER_IDS.PEAKS_SYMBOLS) {
          ...
          if (peakCoordinates) {
            ...
            // Select the peak to open PeakDetailsMap in RouteDetailsControls
            if (onPeakSelect) {
              onPeakSelect(Number(peakId), {
                name: peakName || peakNameEn || "Unknown Peak",
                name_en: peakNameEn || null,
