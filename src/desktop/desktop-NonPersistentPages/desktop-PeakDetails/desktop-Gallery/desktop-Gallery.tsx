...
import styles from "./desktop-Gallery.module.css";
...
const Gallery: React.FC<GalleryProps> = ({ peakId }) => {
  ...
  // Handle delayed button disabling when reaching the end
  useEffect(() => {
    if (!emblaApi) return;

    const canPrev = emblaApi.canScrollPrev();

    // If we can scroll, immediately enable the button
    if (canPrev) {
      setCanScrollPrevDelayed(true);
      return;
    } else {
      // If we can't scroll, wait 1 second before disabling
  ...
  usePeakDetailsView("desktop", "gallery", !loading, peakId);
    ...
    <ShimmerWrapper isLoading={loading} hasData={true}>
      ...
      {!loading && images.length === 0 ? (
        ...
        <div className={styles["gallery__container"]}>
          ...
          {/* Image Counter */}
          {images.length > 1 && (
            <div className={`${styles["gallery__counter"]} typography-desktop-label-medium`}>
