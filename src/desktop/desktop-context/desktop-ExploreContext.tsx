...
import type { AdminArea, AdminSearchResult, DiscoveryMode, ShelterType } from "../../shared/api/types";
...
export interface ExploreContextType {
  ...
  // Selected IDs for UI state (kept for backward compat)
...
export const ExploreProvider: React.FC<ExploreProviderProps> = ({
  ...
  // Backward compat selected IDs (derived from adminLevels)
  const selectedCountryId = adminLevels[0]?.selectedId ?? null;
  const selectedRegionId = adminLevels[1]?.selectedId ?? null;
  ...
  const setSelectedCountryId = useCallback(
    (id: number | null) => selectAdminLevel(0, id),
    [selectAdminLevel]
  );
  const setSelectedRegionId = useCallback(
    (id: number | null) => selectAdminLevel(1, id),
    [selectAdminLevel]
  );
