...
import styles from "./desktop-WikilocRouteMap.module.css";
...
const WikilocRouteMap: React.FC<WikilocRouteProps> = ({
  peakId,
  peakName,
  coordinates,
}) => {
  ...
  usePeakDetailsView("desktop_map", "wikiloc_route", Boolean(peakName), peakId);
      ...
      >
        <div className={styles["wikilocRoute__content"]}>
          ...
          <div className={styles["wikilocRoute__text"]}>
            <h3
              className={`${styles["wikilocRoute__title"]} typography-desktop-body-small`}
            >
              {t("peakDetails.searchRoutesFor")} {peakName}
            </h3>
            <p
              className={`${styles["wikilocRoute__subtitle"]} typography-desktop-label-medium`}
            >
              {t("peakDetails.findHikingRoutesOnWikiloc")}
            </p>
          </div>
...
export default WikilocRouteMap;
