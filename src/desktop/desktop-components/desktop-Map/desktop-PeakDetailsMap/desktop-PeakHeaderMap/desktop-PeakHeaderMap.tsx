...
import styles from "./desktop-PeakHeaderMap.module.css";
import type {
  PeakDetails,
  PeakBasicName,
...
const PeakHeaderMap: React.FC<PeakHeaderProps> = ({ peakId, onLoaded }) => {
  ...
  useEffect(() => {
    ...
    getPeakBasic(peakId)
      ...
      .catch((err) => {
        ...
        console.error("[PeakHeaderMap] Error in getPeakBasic", err);
  ...
  const displayRegion = peak?.reverse_geocoding?.region || getRegionFromHierarchy(peak?.admin_hierarchy);
  const displayCountry =
    peak?.reverse_geocoding?.country || getCountryFromHierarchy(peak?.admin_hierarchy);
  const displayCity = peak?.reverse_geocoding?.city;
  ...
  usePeakDetailsView("desktop_map", "header", Boolean(peak), peakId);

  // Compose location lines
      ...
      <div className={styles["peak-header__details-section"]}>
        ...
        <div className={styles["peak-header__location-block"]}>
          <div className={styles["peak-header__location-text"]}>
            {city && (
              <span
                className={`${styles["peak-header__location-city"]} typography-desktop-body-small`}
              >
                {city}
                {region || country ? "," : ""}
              </span>
            ...
            {region && (
              <span
                className={`${styles["peak-header__location-region"]} typography-desktop-body-small`}
              >
                {region}
                {country ? "," : ""}
              </span>
            ...
            {country && (
              <span
                className={`${styles["peak-header__location-country"]} typography-desktop-body-small`}
              >
                {country}
              </span>
            )}
        ...
        <div className={styles["peak-header__top-row"]}>
          {peak?.elevation && (
            ...
            >
              ...
              />
              <span
                className={`${styles["peak-header__elevation-value-small"]} typography-desktop-body-small`}
              >
