...
import styles from "./desktop-LoginForm.module.css";
  ...
  if (showForgotPassword) {
    return (
      <div className={styles["login-form"]}>
        <h2
          className={`${styles["login-form__title"]} typography-desktop-title-medium`}
        >
          {t("auth.forgotPassword.title")}
        </h2>
        <p
          className={`${styles["login-form__hint"]} typography-desktop-body-small`}
        >
          {t("auth.forgotPassword.instructions")}
        </p>
        {error && (
          <div
            className={`${styles["login-form__error"]} typography-desktop-label-medium`}
          >
            {error}
          </div>
        )}
        <form onSubmit={handleForgotPassword} autoComplete="on">
          ...
          />
          <button
            className={`${styles["login-form__button"]} typography-desktop-button-medium`}
            type="submit"
          ...
          <button
            type="button"
            className={`${styles["login-form__back-to-login"]} typography-desktop-body-small`}
            onClick={() => {
    ...
    >
      ...
      <div className={styles["login-form__field-group"]}>
        ...
        <div className={styles["login-form__links"]}>
          <button
            type="button"
            className={`${styles["login-form__verify-email"]} typography-desktop-body-small`}
          ...
          <button
            type="button"
            className={`${styles["login-form__forgot-password"]} typography-desktop-body-small`}
      ...
      <button
        className={`${styles["login-form__button"]} typography-desktop-button-small`}
        type="submit"
        disabled={loading}
      >
      ...
      <div
        className={`${styles["login-form__divider"]} typography-desktop-body-small`}
      >
      ...
      <div className={styles["login-form__social-buttons"]}>
        <button
          ...
          disabled={loading}
          aria-label={t("auth.loginWithStrava")}
          title={t("auth.loginWithStrava")}
        >
          <img
        ...
        <button
          ...
          disabled={loading}
          aria-label={t("auth.loginWithSuunto")}
          title={t("auth.loginWithSuunto")}
        >
          <img
            src="/icons/wikiloc/suunto.png"
            alt={t("auth.loginWithSuunto")}
            className={styles["login-form__strava-image"]}
          />
        </button>
      </div>
