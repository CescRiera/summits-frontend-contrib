...
import styles from "./desktop-PeakItem.module.css";
import RouteChips from "./desktop-RouteChips.tsx";
...
const PeakItem: React.FC<{ peak: Peak }> = ({ peak }) => {
  ...
  const descriptionBlock = useMemo(() => {
    ...
    const joinWithAnd = (arr: string[], lang: string) => {
      if (!arr || arr.length === 0) return "";
      if (arr.length === 1) return arr[0];
      const andWord =
        { ca: "i", es: "y", en: "and", fr: "et", it: "e" }[lang] || "and";
    ...
    const datesStr = joinWithAnd(
      dates.map((d: string) => formatDate(d, language)),
      language
    );
    const routesStr = joinWithAnd(routes, language);
    ...
    <div className={styles["peak-item"]}>
      <div className={styles["peak-item__main"]}>
        <img
          ...
          src={removeImageSizeRestriction(peak.image) || "/default_peak.png"}
          alt={peak.name}
        />
        <div className={styles["peak-item__info"]}>
          <div
            className={styles["peak-item__name"] + " typography-desktop-label-medium"}
          >
            <img
            ...
            {peak.region && (
              <span
                className={
                  styles["peak-item__region"] + " typography-desktop-label-medium"
                }
              >
                {" "}
                • {peak.region}
          ...
          <div className={styles["peak-item__meta"] + " typography-desktop-label-medium"}>
            <span className="typography-desktop-label-medium">
              {peak.ele ? `${peak.ele}m` : "-"}
            </span>
            {peak.country && (
              <span className="typography-desktop-label-medium"> • {peak.country}</span>
            )}
          </div>
        </div>
        <button
      ...
      {expanded && (
        <div className={styles["peak-item__details"]}>
          <div
            className={
              styles["peak-item__description"] + " typography-desktop-label-medium"
            }
          >
            {descriptionBlock}
          </div>
